...
public class UserServiceTest {
	...
	@Test
	public void testNumberOfUsersCreateData() throws DuplicateInstanceException, InstanceNotFoundException {

		assertEquals(6, userService.findAllSortedByName().size());
	...
	public void testRegisterAndFindUser()
		...
		UserDetails userDetailsTest = userService
				.findUser(userDetails.getUserId());

		assertEquals(userDetailsTest, userDetails);
	...
	public void testUpdateAndFindUser()
		...
		userDetails.setAvatarUrl("url");
		
		exceptionRule.expect(DuplicateInstanceException.class);
		
		userDetails = userService.registerUser("pw", userDetails);
		
		userDetails.setLoginName("u2");
		userDetails = userService.registerUser("pw", userDetails);

		userDetailsTest = userService
				.findUser(userDetails.getUserId());
	...
	@Test
	public void testChangePasswordLogin()
			throws DuplicateInstanceException, InstanceNotFoundException, InvalidDateException, IncorrectPasswordException {
		...
		userDetails = userService.registerUser("old", userDetails);

		userService.changePassword(userDetails.getUserId(), "old", "new");

		exceptionRule.expect(IncorrectPasswordException.class);
		
		userService.login(userDetails.getLoginName(), "old", false);
